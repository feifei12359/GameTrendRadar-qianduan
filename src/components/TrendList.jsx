...
export default function TrendList({ trends }) {
  ...
  if (!sortedTrends.length) {
    return (
      <div className="rounded-3xl border border-dashed border-slate-300 bg-white/70 px-6 py-12 text-center text-sm text-slate-500">
        No trend data yet. Run analysis or the daily job to generate records.
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="grid grid-cols-[1.4fr_0.6fr_0.7fr_0.8fr_1.5fr] gap-4 border-b border-slate-200 bg-slate-50 px-6 py-4 text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">
        <span>Keyword</span>
        <span>Score</span>
        <span>Stage</span>
        <span>Source</span>
      ...
      <div className="divide-y divide-slate-100">
        {sortedTrends.map((trend) => (
          <div
            key={trend.id}
            className="grid grid-cols-[1.4fr_0.6fr_0.7fr_0.8fr_1.5fr] gap-4 px-6 py-5 text-sm text-slate-700"
          >
            <div>
              <div className="font-semibold text-slate-900">{trend.keyword}</div>
              <div className="mt-1 text-xs text-slate-500">{trend.region || 'global'}</div>
            </div>
